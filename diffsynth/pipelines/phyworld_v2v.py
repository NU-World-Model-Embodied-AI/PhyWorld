...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device=get_device_type(), torch_dtype=torch.bfloat16):
        ...
        self.units = [
            ...
            WanVideoUnit_VideoEmbedderVAE_cont(),
    ...
    def __call__(
        ...
        video_start: Optional[list[Image.Image]] = None,
        video_start_frame: Optional[int] = None,
        # Speech-to-video
        ...
        inputs_shared = {
            ...
            "end_image": end_image,
            "input_video": input_video, "denoising_strength": denoising_strength, "video_start": video_start, "video_start_frame": video_start_frame,
     ...
     #   if "first_multi_frame_latents" in inputs_shared:
      #      inputs_shared["latents"][:, :, 0: ( (inputs_shared["video_start_frame"] - 1 ) // 4 + 1) ] = inputs_shared["first_multi_frame_latents"]
...
class WanVideoUnit_VideoEmbedderVAE_cont(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("video_start", "video_start_frame", "num_frames", "height", "width", "tiled", "tile_size",
                          "tile_stride"),
            output_params=("y",),
            onload_model_names=("vae",)
        )

    def process(self, pipe: WanVideoPipeline, video_start, video_start_frame, num_frames, height, width, tiled, tile_size,
                tile_stride):
        if video_start is None or not pipe.dit.require_vae_embedding:
            return {}
        pipe.load_models_to_device(self.onload_model_names)
        video = pipe.preprocess_video(video_start).to(pipe.device)
        video = video.squeeze(0)
        video = video.transpose(0, 1)
        video = video[-video_start_frame:]
        msk = torch.ones(1, num_frames, height // 8, width // 8, device=pipe.device)
        msk[:, video_start_frame:] = 0

        vae_input = torch.concat([video.transpose(0, 1), torch.zeros(3, num_frames - video_start_frame, height, width).to(video.device)], dim=1)

        msk = torch.concat([torch.repeat_interleave(msk[:, 0:1], repeats=4, dim=1), msk[:, 1:]], dim=1)
        msk = msk.view(1, msk.shape[1] // 4, 4, height // 8, width // 8)
        msk = msk.transpose(1, 2)[0]

        y = pipe.vae.encode([vae_input.to(dtype=pipe.torch_dtype, device=pipe.device)], device=pipe.device, tiled=tiled,
...
class WanVideoUnit_VideoEmbedderFused(PipelineUnit):
    """
    Encode input video to latents using VAE. This unit is for Wan-AI/Wan2.2-TI2V-5B.
    """
    def __init__(self):
        super().__init__(
            input_params=("video_start", "video_start_frame", "latents", "height", "width", "tiled", "tile_size", "tile_stride"),
            output_params=("latents", "fuse_vae_embedding_in_latents", "first_multi_frame_latents"),
            onload_model_names=("vae",)
        )

    def process(self, pipe: WanVideoPipeline, video_start, video_start_frame, latents, height, width, tiled, tile_size, tile_stride):
        if video_start is None or not pipe.dit.fuse_vae_embedding_in_latents:
            return {}
        pipe.load_models_to_device(self.onload_model_names)
        video = pipe.preprocess_video(video_start).to(pipe.device)
        video = video.squeeze(0)
        video = video.transpose(0, 1)
        video = video[-video_start_frame:]
        z = pipe.vae.encode([video.transpose(0, 1)], device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        latents[:, :, 0: ((video_start_frame-1)//4 + 1)] = z
        return {"latents": latents, "fuse_vae_embedding_in_latents": True, "first_multi_frame_latents": z}
